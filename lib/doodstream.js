...
const Doodstream = async (url) => {
    try {
        ...
        if (!id) throw new Error('Link tidak dapat diproses. Silakan ganti dengan link baru.');
        ...
        if (!cdn) {
            console.error('CDN tidak ditemukan:', dp);
            return { error: "Link Pass MD5 tidak ada. Coba lagi nanti." };
        }

        const chars = Array.from(crypto.randomBytes(10))
